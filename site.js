async function startCheckout(product, variant) {
  try {
...
async function getProductDetails(productId) {
  ...
  if (!response.ok) {
    throw new Error(
      data.error || "Unable to load product sizes"
    );
  }
      ...
      .map((p, index) => {
        ...
        return `
          ...
          >
            ...
            <p>
              Official Everlight Chronicles merchandise.
            </p>
    ...
    await Promise.all(
      products.map(async (product, index) => {
        ...
        try {
          ...
          select.innerHTML =
            '<option value="">Sizes unavailable</option>';
        }
      })
    );
  } catch (error) {
